...
(function ($) {
    ...
    if (has_canvas) {
        ...
        var draw_shape = function (context, shape, coords, x_shift,
                                   y_shift) {
            x_shift = x_shift || 0;
            y_shift = y_shift || 0;
            context.beginPath();
            if (shape == "rect")context.rect(coords[0] + x_shift, coords[1] + y_shift, coords[2] - coords[0], coords[3] - coords[1]); else if (shape == "poly") {
                context.moveTo(coords[0] + x_shift, coords[1] + y_shift);
                for (i = 2; i < coords.length; i += 2)context.lineTo(coords[i] + x_shift, coords[i + 1] + y_shift)
            } else if (shape == "circ")context.arc(coords[0] + x_shift, coords[1] + y_shift, coords[2], 0, Math.PI * 2, false);
            context.closePath()
        };
        add_shape_to = function (canvas, shape, coords, options,
            ...
            if (options.shadow) {
                ...
                var shadowFrom =
                    options.shadowFrom;
                if (!shadowFrom)if (options.shadowPosition == "outside")shadowFrom = "fill"; else shadowFrom = "stroke";
            ...
            context.restore();
            if (options.fade)$(canvas).css("opacity", 0).animate({opacity: 1}, 100)
        };
        ...
        create_canvas_for = function (img) {
            return $('<var style="zoom:1;overflow:hidden;display:block;width:' +
                img.offsetWidth + "px;height:" + img.offsetHeight + 'px;"></var>').get(0)
        };
        add_shape_to = function (canvas, shape, coords, options, name) {
            ...
            if (shape == "rect")e =
                ...
                canvas.width + "px;height:" + canvas.height + 'px;"></v:shape>'); else if (shape == "circ")e = $('<v:oval name="' + name + '" filled="t" ' + stroke + ' style="zoom:1;margin:0;padding:0;display:block;position:absolute;left:' + (coords[0] - coords[2]) + "px;top:" + (coords[1] - coords[2]) + "px;width:" + coords[2] * 2 + "px;height:" + coords[2] * 2 + 'px;"></v:oval>');
    ...
    $.fn.maphilight = function (opts) {
        ...
        return this.each(function () {
            ...
            img = $(this);
            if (!is_image_loaded(this))return window.setTimeout(function () {
                img.maphilight(opts)
            }, 200);
            ...
            if (!(img.is('img,input[type="image"]') && usemap && map.size() > 0))return;
            ...
            if (options.wrapClass)if (options.wrapClass === true)wrap.addClass($(this).attr("class")); else wrap.addClass(options.wrapClass);
            img.before(wrap).css("opacity", 0).css(canvas_style).remove();
            if (has_VML)img.css("filter", "Alpha(opacity=0)");
            wrap.append(img);
            $("div.map").css("background-size", "cover");
            ...
            mouseover = function (e) {
                ...
                if (!area_options.neverOn && !area_options.alwaysOn) {
                    ...
                    if (area_options.groupBy) {
                        var areas;
                        if (/^[a-zA-Z][\-a-zA-Z]+$/.test(area_options.groupBy))areas =
                            map.find("area[" + area_options.groupBy + '="' + $(this).attr(area_options.groupBy) + '"]'); else areas = map.find(area_options.groupBy);
                        var first = this;
                        areas.each(function () {
                            if (this != first) {
            ...
            $(map).bind("alwaysOn.maphilight", function () {
                ...
                $(map).find("area[coords]").each(function () {
                    ...
                    if (area_options.alwaysOn) {
                        ...
                        shape = shape_from_area(this);
                        if (has_canvas)add_shape_to(canvas_always, shape[0],
                            shape[1], area_options, ""); else add_shape_to(canvas, shape[0], shape[1], area_options, "")
                    }
                });
            });
	...
	$(function() {
		$('ul.tabs_caption').on('click', 'li:not(.active)', function() {
			$(this)
				...
				.closest('nav.product_specs_tabs').find('div.tabs_content').removeClass('active').eq($(this).index()).addClass('active');
		});
	});
})(jQuery);
...
(function($) {
	$(function() {
		$('ul.tabs_caption').on('click', 'li:not(.active)', function() {
			$(this)
				.addClass('active').siblings().removeClass('active')
				.closest('nav.user-room_tabs').find('div.tabs_content').removeClass('active').eq($(this).index()).addClass('active');
		});
	});
})(jQuery);

/* табы в логин-регистрация */
(function($) {	

	$('.main-header__login').magnificPopup({
		type:"inline"
	});	

	$('.main-header__reg').magnificPopup({
		type:"inline"
	});	

	$(".main-header__login").click(function(){
		$('.log-tab').addClass('active');
		$('.log-content').addClass('active');
		$('.reg-tab').removeClass('active');
		$('.reg-content').removeClass('active');
	});	

	$(".main-header__reg").click(function(){
		$('.reg-tab').addClass('active');
		$('.reg-content').addClass('active');
		$('.log-tab').removeClass('active');
		$('.log-content').removeClass('active');
	});
	...
	$(function() {
		$('ul.tabs_caption').on('click', 'li:not(.active)', function() {
			$(this)
				.addClass('active').siblings().removeClass('active')
				.closest('nav.login_tabs').find('div.tabs_content').removeClass('active').eq($(this).index()).addClass('active');
		});
	});
})(jQuery);
...
$(function() {
	...
	$('ul.about-tabs-caption').on('click', '.about-tabs-caption__item:not(.active)', function() {
		$(this)
		.addClass('active').siblings().removeClass('active')
		.closest('.main-content').find('.tabs-content').removeClass('active').eq($(this).index()).addClass('active');
	});
	...
	$('.cooperations-anchors__link').scrollToID({
	  speed: 1000,
	  offset: 30   
	});
	...
	$(window).resize(function() {
		...
		$(".clients-slider, .review-slider").slick('unslick');
		$(".clients-slider").slick({
			speed: 300,
			slidesToShow: 4,
			slidesToScroll: 1,
...
$(function() {
	...
	// 		if($(form).hasClass('popup_form_price')){
	// 			$(location).attr('href', linkP);
	// 		}
	// 		if($(form).hasClass('form_price')){
	// 			$(location).attr('href', 'files/price.xlsx');
	// 		}
	// 		setTimeout(function(){
	...
	$('.main-footer__callback-btn').magnificPopup({
		type:"inline"
	});	
	
	$('.main-product_buy-info_one-click-btn').magnificPopup({
		type:"inline"
	});	

	$('.main-product_buy-info_message').magnificPopup({
		type:"inline"
	});	


	$(function() {
		$('ul.sort-block_cost').on('click', 'li.sort-block_cost-item:not(.active)', function() {
			$(this).addClass('active').siblings().removeClass('active');
		});
	});

	$(function() {
		$('ul.sort-block_rating').on('click', 'li.sort-block_rating-item:not(.active)', function() {
			$(this).addClass('active').siblings().removeClass('active');
		});
	});
	
	$(function(){
		$('.cat_scroll_wrapper').jScrollPane();
	});
	...
	setTimeout(function() {
		...
		setTimeout(function(){
			$(".main-product_gallery-slider .slick-track").lightGallery({
				thumbnail:true,
				download:false,
				selector: 'img'
			});
	...
	/*увеличение/уменьшение схемы*/
	$('.scale-block_plus').click(function(e){
		e.preventDefault();
		var parent = $(this).parents('.tab-pane');
		parent.find('.scheme-block').addClass('scale');
		parent.find('.scheme-block-wrapper').css('overflow', 'scroll');
		parent.find('.scale-block_minus').removeClass('active');
		$(this).addClass('active');
		parent.find('.scale-block_title').text('Масштаб 150%');
	});

	$('.scale-block_minus').click(function(e){
		e.preventDefault();
		var parent = $(this).parents('.tab-pane');
		parent.find('.scheme-block').removeClass('scale');
		parent.find('.scheme-block-wrapper').css('overflow', 'hidden');
		parent.find('.scale-block_plus').removeClass('active');
	  	$(this).addClass('active');
		parent.find('.scale-block_title').text('Масштаб 100%');
	});

	$('.scale-block_clear').click(function(e){
		e.preventDefault();
	  $('.scheme-block_checkbox').removeAttr("checked");
	});
	

	/*показать/скрыть содержимое сайдбара*/
	if ($('.recommend').hasClass('showHideContent')){
			$('.recommend').removeClass('showHideContent');	
	}
	if($(window).width() <= 767){

		if (!$('.recommend').hasClass('showHideContent')){
			$('.recommend').addClass('showHideContent');	
		}
		$('.showHideContent').removeClass('show');
	...
	/*счетчик в каталоге*/
	$('#cart_content, #order_cart_content, .cat-block, .main-content.product').on('click', '.minus', function () {
  ...
  $('#cart_content, #order_cart_content, .cat-block, .main-content.product').on('click', '.plus', function () {
      var $input = $(this).parent().find('input');
	...
	/* меню каталог на мобильных устройствах*/
	if($(window).width() < 767){
		$('.cat_menu-item').click(function(e){
				e.preventDefault();
				$(this).next('ul').slideToggle(250);
		});

		$('.cat_submenu-item').click(function(e){
				e.preventDefault();
		});
	...
	$(".special_offers-ref_link").click(function(){
		$(this).select();
	});

	/*удаление из корзины*/
	$(document).ready(function() {
		if ($('.cart-block_item').length != 0){
			$('.cart-block_checkout').show();
		}
		else{
			$('.cart-block_checkout').hide();
			$('.empty-cart').css('display', 'flex');
		}
	});

	// $('.cart-block_remove-item').click(function () {
	// 	$(this).parent().find('a').toggle('slow').promise().done(function () {
  ...
  /*отзыв полезен да/нет*/
 	// $('.review_helpfulness').likeDislike({
    //     reverseMode: false,
    ...
    //         $('button.active').on("click", function(event) {
	 ...
	 //            	$(this).children().stop(true).fadeIn().fadeOut(1500);
    //
	 //            }
    //         });
    //
  ...
  /*добавить отзыв*/
	$('.hide_review_form').click(function(){
		if ($(this).hasClass('add_review_btn')){
			$(this).addClass('add_review_form_title');
			$(this).removeClass('add_review_btn');
		  $('.hidden_review_form').slideToggle();
		}
	});

	$('.cancel_review_btn').click(function(){
	  $('.hidden_review_form').slideToggle();
	  $('.hide_review_form').addClass('add_review_btn');
		$('.hide_review_form').removeClass('add_review_form_title');
	});
	...
	/*показать/скрыть блок регистрации в корзине*/

	$(".cart_register").click(function(){
		$('.cart_reg_wrap').slideToggle();
	});

	/*показать/скрыть фильтры*/

	// $('.more_filters-list').readmore({
	// 	speed: 300,
	// 	maxHeight: 120,
	// 	moreLink: '<a class="more_filters" href="#">Показать еще</a>',
	// 	lessLink: '<a class="less_filters" href="#">Свернуть</a>'
	// });
...
jQuery(document).ready(function($){
    ...
    $('#add-like, #add-dislike').on('click', function(){
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.error){
                    $this.find('.error-message').css('display', 'block').text(response.error);
                    setTimeout(function(){$this.find('.error-message').fadeOut(300);}, 1000);
                } else if(response.success) {
                    parent.find('.likes').text(response.like);
                    parent.find('.dislikes').text(response.dislike);
                    $this.find('.error-message').css('display', 'block').text(response.success);
                    setTimeout(function(){$this.find('.error-message').fadeOut(300);}, 1000);
                }
            }
        });
    ...
    $(document).on('click', '.btn_buy', function (e) {
        ...
        if(!$(this).hasClass('disable')) {
            $('.main-header__cart-wrap').removeClass('shake');
            ...
            data.quantity = $(this).parent().find('.count_field').val();
        ...
        }).text('Оформить').addClass('disable').css('background-color', '#009c40');
    });

    /**
     * Удаление товара из корзины
     */
    $(document).on('click', '.cart-block_remove-item', function () {
        var $this = $(this);
        update_cart({
            action: 'remove',
            product_id: $this.data('product-id')
    ...
    function update_cart(data){
        $.post("/cart/update", data, function(cart){
            ...
            var quantity = $(cart).find('#total_quantity').val();

            if(quantity > 0){
                $('.main-header__cart-wrap').append('<span class="main-header__cart-items">'+quantity+'</span>').addClass('shake');
            }else{
                $('.main-header__cart-wrap .main-header__cart-items').remove();
                $('.cart-block_checkout').css('display', 'none');
            }
        });
    }
    ...
    //     e.preventDefault();
    //     var delivery = $('#checkout-delivery-payment');
    //
    //     $.ajax({
    ...
    //         type: 'post',
    //         beforeSend: function() {
    //             $('span.error-message').html('').addClass('hidden');
    //             $('input, select + .jq-selectbox__select').removeClass('input_error');
    //             delivery.addClass('loader');
    //         },
    //         success: function(response) {
    //             if (response.error) {
    //                 html = '';
    ...
    //                     html += error + '<br />';
    //                     $('input[name="' + index + '"], select[name="' + index + '"] + .jq-selectbox__select').addClass('input_error');
    //                 });
    //                 $('span.error-message').html(html).removeClass('hidden');
    //                 delivery.removeClass('loader');
    //             } else if (response.success) {
    //                 if (response.success == 'liqpay') {
    //                     $('body').prepend(
    //                         '<form method="POST" id="liqpay-form" action="' + response.liqpay.url + '" accept-charset="utf-8">' +
     ...
     */
    $('#checkout-step__delivery').fancySelect().on('change.fs', function(){
        if ($(this).val() != 0) {
            $('.checkout-step__body').addClass('checkout-step__body_loader');
    ...
    // Пагинация
    var paginate_btn = $('#paginate');
    if(paginate_btn.length){
        window.loading = false;
        paginate_btn.click(function(e){
            e.preventDefault();
            paginate($(this));
        });
        $(window).scroll(function(){
            var scroll=$(window).scrollTop()+$(window).height()*2;
            if(paginate_btn.offset().top < scroll)
                paginate(paginate_btn);
        });
        function paginate(btn){
            var url = btn.data('alias');
            var page = parseInt(btn.data('current-page')) + 1;
            var sort = btn.data('sort');
            var search = btn.data('search');
            if(!window.loading) {
                window.loading = true;
                $.get(url, {page: page, sort: sort, search: search, json: true}, function (response) {
                    btn.before(response);
                    if (btn.data('pages-count') > page) {
    ...
    $(window).resize(function () {
        $('.tab-pane.active img[usemap]').maphilight();
    });
    setTimeout(function(){
        $('area').each(function(){
        ...
        $('.map-all.map canvas').css('display', 'inline-block');
    }, 200);

    $('#schemes').on('shown.bs.tab', 'a[data-toggle="tab"]', function (e) {
        $('.tab-pane.active img[usemap]').rwdImageMaps();
        setTimeout(function(){
            $('area').each(function(){
                var d = $(this).data('maphilight') || {};
                if(d.alwaysOn)
    ...
    $('.scheme_map area').hover(
        function(){
            var map = $(this).parents('map');
            var group = map.find('.'+$(this).attr('class'));
            group.each(function(){
                setPointColor($(this), 'E02000');
            });
        },function(){
            var map = $(this).parents('map');
            var group = map.find('.'+$(this).attr('class'));
            group.each(function(){
                if(!$(this).hasClass('active'))
                    setPointColor($(this), '009C40');
            });
        }
    );

    // Выбор товара на схеме
    $('.scheme_map area').click(function(){
        var $this = $(this);
        var map = $this.parents('map');
        ...
        $.post('/get_product', {id:id}, function(response){
            var container = $('#selected_product_container');
            if ($(container).length != 0) {
                container.html(response);
                var scroll = $(container).offset().top - $(window).height() + container.height() + 50;
    ...
    function setPointColor(obj, color){
        var d = obj.data('maphilight') || {};
        d.strokeColor = color;
        d.fillColor = color;
        obj.data('maphilight', d).trigger('alwaysOn.maphilight');
    }

    $('.show_all_products').click(function(){
        //$('.product.hidden').removeClass('hidden');
        $('#selected_product_container').html('');
        $('area').each(function(){
            setPointColor($(this), '009C40');
        });
        var scroll_el = $('.cat-block');
    ...
    $('.cities-select').fancySelect().on('change.fs', function(){
        $.cookie('selected_city', $(this).val());
        window.location = window.location.origin+window.location.pathname;
    });

    $('#scheme_select').fancySelect().on('change.fs', function(){
        var id = $(this).val();
        if(id == '#scheme_hide'){
            $('a[href='+id+']').tab('show');
            return true;
        }
        var img = $(id).find('img.map-all');
        ...
        if(typeof src == 'undefined'){
            ...
            $(id).find('img.print').attr('src', img.data('src'));
        }else
            $('a[href='+id+']').tab('show');
    });

    /**
     * Обратный звонок, Купить в один клик
     */
    $('.recall, #one-click-buy').submit(function(e){
        e.preventDefault();
        $this = $(this);
        $this.find('.error-message').empty();

        $.post('/recall', $(this).serialize(), function(response){
            if(response.success){
                $this.find('input').val('');
                $this.find('.error-message').append('<p>Запрос принят в обработку, в ближайшее время наш менеджер свяжется с вами.</p>');
                setTimeout(function(){
            ...
            if(typeof response.error !== 'undefined'){
                for(err in response.error){
                    for(error in response.error[err]){
                        $this.find('.error-message').append('<p>'+response.error[err][error]+'</p>');
                    }
                }
            }
        }, 'json');
    ...
    /**
     * Сообщение в контактах
     */
    $('.contacts_message').submit(function(e){
        e.preventDefault();
        $this = $(this);
        $this.find('.error-message').empty();

        $.post('/contacts_message', $(this).serialize(), function(response){
            if(response.success){
     ...
     * Сообщение в контактах
     */
    $('.sale_subscribe').submit(function(e){
        e.preventDefault();
        $this = $(this);
        $this.find('.error-message').empty();

        $.post('/sale_subscribe', $(this).serialize(), function(response){
            if(response.success){
                $this.find('input, textarea').val('');
                $this.find('.error-message').append('<p>Запрос принят в обработку, в ближайшее время наш менеджер свяжется с вами.</p>');
            }
    ...
    /**
     * Сообщить, когда цена снизиться
     */
    $('#sale_price_request').submit(function(e){
        e.preventDefault();
        $this = $(this);
        $this.find('.error-message').empty();

        $.post('/sale_price_request', $(this).serialize(), function(response){
            if(response.success){
                $this.find('input').val('');
                $this.find('.error-message').append('<p>Запрос принят в обработку, в случае снижения цены, Вы будете оповещены по e-mail.</p>');
                setTimeout(function(){
                    $this.magnificPopup('close');
                }, 3000);
            }
     ...
     * Авторизация
     */
    $('.form-login, .form-reg').submit(function(e){
        e.preventDefault();
        $this = $(this);
        $this.find('.error-message').empty();
    ...
    $(document).scroll(function(){
        if( $(document).scrollTop() > $('.main-header').height() + 10 ){
            $('.main_nav, .search-block').addClass('fixed');
        }else{
            $('.main_nav, .search-block').removeClass('fixed');
        }
    });
    if( $('html, body').scrollTop() > $('.main-header').height() + 10 ){
        $('.main_nav, .search-block').addClass('fixed');
    }else{
        $('.main_nav, .search-block').removeClass('fixed');
    }
